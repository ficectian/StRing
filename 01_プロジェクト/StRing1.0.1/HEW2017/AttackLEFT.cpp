...
//=========================================================
void  AttackLEFT_Display(int Pt_tate, int Pt_yoko){
	int Attack_Out[ATTACK_HEIGHT_HEIGHT][ATTACK_HEIGHT_WIDTH];		//Player画面変数：横：５；縦：１２

	//Player画面データを読み込む
	FILE *fp;
	fp = fopen(FILENAME_ATTACKLEFT, "rt+");
	...
	for (int tate = 0; tate < ATTACK_HEIGHT_HEIGHT; tate++){
		for (int yoko = 0; yoko < ATTACK_HEIGHT_WIDTH; yoko++){
			LOCATE(Pt_yoko -2+ (2 * yoko) - ATTACK_HEIGHT_WIDTH*2, Pt_tate + tate);
			//１６の時背景色を表示
...
void  AttackLEFT_W_Display(int Pt_tate, int Pt_yoko){
	int Attack_Out[ATTACK_HEIGHT_HEIGHT][ATTACK_HEIGHT_WIDTH];		//Player画面変数：横：５；縦：１２
...
//	引数：int： *Scene（背景画面）
//=========================================================
void  AttackCLLEFT_Display(int Pt_tate, int Pt_yoko, const char   *SCENE){
	int Attack_Out[ATTACK_HEIGHT_HEIGHT][ATTACK_HEIGHT_WIDTH];		//Player画面変数：横：５；縦：１２
	...
	for (int tate = 0; tate < ATTACK_HEIGHT_HEIGHT; tate++){
		for (int yoko = 0; yoko < ATTACK_HEIGHT_WIDTH; yoko++){
			...
			//１６の時背景色を表示
			if (Attack_Out[tate][yoko] != 16){
				if (Scene[Pt_tate + tate][Pix_YOKO-1 +  yoko - ATTACK_HEIGHT_WIDTH ] == 16){
					COLOR(BACKGROUND_COLOR, BACKGROUND_COLOR);
					printf("　");
				}
				if (Scene[Pt_tate + tate][Pix_YOKO-1 + yoko - ATTACK_HEIGHT_WIDTH] == 0){		//Scene + WINDOWS_WIDTH	 * (Pt_tate + tate) + (Pt_yoko + yoko)
					//Scene[Pt_tate + tate][Pt_yoko + yoko]
					COLOR(L_BLACK, L_BLACK);
					printf("　");
				}
				else{
					COLOR(Scene[Pt_tate + tate][Pix_YOKO-1 + yoko - ATTACK_HEIGHT_WIDTH], Scene[Pt_tate + tate][Pix_YOKO-1 + yoko - ATTACK_HEIGHT_WIDTH]);
					printf("　");
				}
			}
		}
